...
module SiriusApi
  ...
  class BaseAuthorizer
    ...
    class << self
      ...
      #
      # Calling `.permit` inside the given block will define a new access rule for all
      ...
      #
      # @param http_method [Symbol] Lowercase name of HTTP method for which is the rule defined,
      #   e.g. `:get`, `:post` etc.
      ...
      #   If set, the access will be only allowed if the block returns true.
      #
      ...
      ##
      # @return [Hash] all rules defined by permission DSL.
      def scope_registry
        @scope_registry ||= {}
      end
    end
    ...
    #
    # @param current_user [SiriusApi::User] User object representing the user currently accessing
    #   the application.
    #
    ...
    #
    # @param http_method [Symbol] Lowercase name of currently performed HTTP method,
    #   e.g. `:get`, `:post` etc.
    ...
    # @raise [SiriusApi::Errors::Authorization] if access is denied.
    #
