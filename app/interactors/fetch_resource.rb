...
class FetchResource < KOSapiInteractor
  ...
  def perform(resource:, faculty_semester:, limit: 100, paginate: true)
    @rooms = {}
    @faculty_semester = faculty_semester
    client = kosapi_client(faculty_semester)
    @exams = client.send(resource).where("course.faculty==#{faculty_semester.faculty};semester==#{faculty_semester.code}").limit(limit)
